...
parser.add_argument('--decay_freq', type=int, default=50)
...
# Classifier training
logger.info('Start Classfier train...')
...
for MODEL_NUMBER in range(args.start_model_num, args.model_num, 1):
    ...
    optimizer = torch.optim.Adam(classifier.parameters(), lr=args.init_lr)
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, args.decay_freq, gamma=0.5)
    # scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, 8, eta_min=1e-8)
    classifier = classifier.train()
    iteration = 0
    break_count = 0
    best_loss = 10000000
    stop_sign = 0
    for epoch in range(args.total_epcho_num):
        print(optimizer.state_dict()['param_groups'][0]['lr'])
        shape_ious = []
        for data in dataloader:
            # print(f"Preparing the train set for {opts['categories']}...")
            X = torch.zeros((args.batch_size, *opts['dim']), dtype=torch.float)
            y = torch.zeros((args.batch_size, *opts['dim'][:-1]), dtype=torch.uint8)
            ...
            # print('X,  ', X.shape)
            y = (data[2]-args.label_min).to(device=opts['device'],dtype=torch.uint8)#.repeat(args.batch_size,1)
            y_bk = (data[2]-args.label_min).to(device=opts['device'],dtype=torch.uint8)#.repeat(args.batch_size,1)
    #         d = X.shape[2]
    #         X = X.permute(2,0,1).reshape(d, -1).permute(1, 0)
    #         y = y.flatten()
    #         features, labels = X, y
            ...
            print('MODEL_NUMBER = ', MODEL_NUMBER, 'epoch = ', epoch, 'iter = ', iteration, 'loss = ', loss.item())
            ...
            if iteration % 100 == 0:
                ...
                y_pred_softmax = y_pred_softmax.transpose(2,1)
                y_pred_tags = y_pred_softmax.data.max(2)[1]
                pred_np = y_pred_tags.cpu().data.numpy()
                target_np = y_bk.cpu().data.numpy()# - 1
                for shape_idx in range(args.batch_size):
                    parts = range(args.num_class)#np.unique(target_np[shape_idx])
                    part_ious = []
                    for part in parts:
                    ...
                    obj_shape_ious.append(np.mean(part_ious))

                logger.info('MODEL_NUMBER:  %d  , Epoch:  %s  , iteration %d , mio  %.4f' % (MODEL_NUMBER, str(epoch), iteration,  np.mean(obj_shape_ious)))
            iteration += 1
        scheduler.step()
        model_path = os.path.join(save_dir, 'model_' + str(MODEL_NUMBER) + '.pth')
...
print('test set len = ', len(dataset))
shape_ious = []
shape_acc = []
for data in testdataloader:
    X = torch.zeros((args.batch_size, *opts['dim']), dtype=torch.float)
    y = torch.zeros((args.batch_size, *opts['dim'][:-1]), dtype=torch.uint8)
    pc = (data[0]).to(device=opts['device'],dtype=torch.float32)
    features = feature_extractor(pc, noise=noise)
    X = collect_features_batch(opts, features).cpu()
    y = (data[2]-args.label_min).to(device=opts['device'],dtype=torch.uint8)
    y_bk = (data[2]-args.label_min).to(device=opts['device'],dtype=torch.uint8)
            
    X_batch, y_batch = X.permute(0,2,1).to(args.device), y.to(args.device)
    y_batch = y_batch.type(torch.long)
    y_pred = classifier(X_batch)

    y_pred = y_pred.transpose(2,1).contiguous()
    y_pred = torch.log_softmax(y_pred.view(-1,args.num_class), dim=-1)
    y_pred = y_pred.view(args.batch_size, 2048, args.num_class)
    y_pred = y_pred.view(-1, args.num_class)
    pred_choice = y_pred.data.max(1)[1]
    target = y_bk.view(-1, 1)[:, 0]
    correct = pred_choice.eq(target.data).cpu().sum()
    accuracy = correct.item()/float(args.batch_size * 2048)
    shape_acc.append(accuracy)
print('shape_acc.len = ', len(shape_acc))
dataset = FewShotPartNormalDataset(root=args.dataset_path, split='test', normalize=args.dataset_normalize, npoints=2048, cates=args.categories, k_shot=-1)
testdataloader = DataLoader(dataset,batch_size=args.batch_size,shuffle=True,drop_last=True)
for data in testdataloader:
    X = torch.zeros((args.batch_size, *opts['dim']), dtype=torch.float)
    ...
    # print('iou, ', pred_np.shape, target_np.shape) iou,  (4, 2500) (4, 2500)
    y_pred_softmax = torch.log_softmax(y_pred, dim=1)
    y_pred_softmax = y_pred_softmax.transpose(2,1)
    y_pred_tags = y_pred_softmax.data.max(2)[1]
    pred_np = y_pred_tags.cpu().data.numpy()
...
logger.info("mIOU for class {}: {}, mAcc: {}".format(args.categories, np.mean(shape_ious), np.mean(shape_acc)))    
