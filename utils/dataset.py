...
if __name__ == '__main__':
    d = ShapeNetCore(path='/data2/tli/normal-auxiliary-diffusion-point-cloud/data/shapenet.hdf5',
    cates=['chair'],
    split='train',
    ...
    for i in range(5):
        ps = d[i]['pointcloud']
        print(ps.shape)
        print(ps.max().item(), ps.min().item(), ps.mean().item(), ps.std().item())
        print(ps[:,0].max().item(), ps[:,0].min().item(), ps[:,0].mean().item(), ps[:,0].std().item())
        print(ps[:,1].max().item(), ps[:,1].min().item(), ps[:,1].mean().item(), ps[:,1].std().item())
        print(ps[:,2].max().item(), ps[:,2].min().item(), ps[:,2].mean().item(), ps[:,2].std().item())
