...
class PartNormalDataset():
    def __init__(self, root, npoints = 2500, classification = False, split='train', normalize=True, return_cls_label = False, cates='chair'):
...
if __name__ == '__main__':
    d = PartNormalDataset(root = '/data2/tli/normal-auxiliary-diffusion-point-cloud/data/shapenetcore_partanno_segmentation_benchmark_v0_normal', split='train', npoints=3000, cates='car')
    print('len(d) = ', len(d))
    ...
    #     color = np.ones((3000,4))
    #     for j in range(3000):
    #         if seg[j] == 12.0:
    #             color[j] = np.array([1,0,0,1])
    #         if seg[j] == 13.0:
    #             color[j] = np.array([0,1,0,1])
    #         if seg[j] == 14.0:
    #             color[j] = np.array([0,0,1,1])
    #         if seg[j] == 15.0:
    #             color[j] = np.array([0.5,0,0.5,1])
    #     pcu.save_mesh_vnc(f'./reg{i}.ply',ps,normal,color)
